...
public:
    CameraAPI();
    ~CameraAPI();
    ActionInfo connect(const char* deviceName);
    ...
    std::list<PreviewSize> getPreviewSize();
    ActionInfo setFrameSize(int width, int height, int frame_format, int denominator);
