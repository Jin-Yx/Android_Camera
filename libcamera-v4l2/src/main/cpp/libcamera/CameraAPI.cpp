...
void CameraAPI::sendFrame(JNIEnv *env, uint8_t *data) {
    if (frameCallback_onFrame && LIKELY(data)) {
        ...
        env->CallVoidMethod(frameCallback, frameCallback_onFrame, frameFormat, frameWidth, frameHeight, frame);
...
ActionInfo CameraAPI::connect(const char* deviceName) {
...
std::list<PreviewSize> CameraAPI::getPreviewSize() {
    struct v4l2_fmtdesc fmtd;   // 存储摄像头支持的传输格式
    struct v4l2_frmsizeenum frmsize;  // 存储摄像头支持的分辨率
    struct v4l2_frmivalenum framival; // 存储分辨率支持的帧率

    PreviewSize previewSize;
    std::list<PreviewSize> sizes;

    for (int i = 0; ; i++) {
        fmtd.index = i;
        fmtd.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
        if (0 > ioctl(fd, VIDIOC_ENUM_FMT, &fmtd)) {
            break;
        }
        previewSize.pixelFormat = fmtd.pixelformat;

        for (int j = 0; ; j++) {
            frmsize.index = j;
            frmsize.pixel_format = fmtd.pixelformat;
            if (0 > ioctl(fd, VIDIOC_ENUM_FRAMESIZES, &frmsize)) {
                break;
            }
            previewSize.width = frmsize.discrete.width;
            previewSize.height = frmsize.discrete.height;
            previewSize.frameRates.clear();
            ...
            for (int k = 0; ; k++) {
                ...
                if (0 > ioctl(fd, VIDIOC_ENUM_FRAMEINTERVALS, &framival)) {
                    break;
                }
                FrameRate frameRate;
                frameRate.numerator = framival.discrete.numerator;
                frameRate.denominator = framival.discrete.denominator;
                previewSize.frameRates.push_back(frameRate);
            }

            sizes.push_back(previewSize);
        }
    }
    return sizes;
}

ActionInfo CameraAPI::setFrameSize(int width, int height, int frame_format, int denominator) {
    if (STATUS_OPEN == getStatus()) {
        ...
        if (!env->IsSameObject(frameCallback, frame_callback)) {
            ...
            if (frame_callback) {
                ...
                if (LIKELY(clazz)) {
                    ...
                    frameCallback_onFrame = env->GetMethodID(clazz, "onFrame","(IIILjava/nio/ByteBuffer;)V");
                }
                env->ExceptionClear();
